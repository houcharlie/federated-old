...
Mfedavg_Mmbsgd_fn = lambda lr, switch: f'/ocean/projects/iri180031p/houc/multistage/long_exp/results/EMNISTMULTI_{lr}_switch_{switch}_control_0_allowswap_1'
Mscaffold_Mmbsgd_fn = lambda lr, switch: f'/ocean/projects/iri180031p/houc/multistage/long_exp/results/EMNISTMULTI_{lr}_switch_{switch}_control_1_allowswap_1'
Mfedavg_fn = lambda lr, switch: f'/ocean/projects/iri180031p/houc/multistage/long_exp/results/EMNISTMULTI_{lr}_switch_{switch}_control_0_allowswap_0'
Mscaffold_fn = lambda lr, switch: f'/ocean/projects/iri180031p/houc/multistage/long_exp/results/EMNISTMULTI_{lr}_switch_{switch}_control_1_allowswap_0'
...
def plot_metric(metric, 
    ...
    def find_best_in_grid(lr_grid, switch_grid, exp_name_fn):
        ...
        for lr in lr_grid:
            for switch in switch_grid:
                ...
                final_y = min(metric_fn(df[metric]))
                if final_y < best_val:
                    best_val = final_y
                    best_file = filename
        print(best_file)
    ...
    best_file = find_best_in_grid([0.1, 0.2, 0.30000000000000004, 0.4, 0.5], [0.01, 0.023713737056616554, 0.05623413251903491, 0.1333521432163324, 0.31622776601683794], Mmbsgd_fn)
    df = pd.read_csv(best_file, sep=',')
    y = metric_fn(df[metric])[::10]
    ...
    best_file = find_best_in_grid(lr_grid, multistage_first_grid, Mfedavg_Mmbsgd_fn)
    df = pd.read_csv(best_file, sep=',')
    y = metric_fn(df[metric])[::10]
    ...
    best_file = find_best_in_grid(lr_grid, multistage_first_grid, Mscaffold_Mmbsgd_fn)
    df = pd.read_csv(best_file, sep=',')
    y = metric_fn(df[metric])[::10]
